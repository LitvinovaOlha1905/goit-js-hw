...
function onSubmitForm(evt) {
    evt.preventDefault();
    const { elements: { delay: delay1 , step , amount  } ,  } = evt.currentTarget;
    if (delay1.value < 0 || step.value < 0 || amount.value < 0) (0, _notiflixDefault.default).Notify.warning(`❗ Please enter a positive number`);
    else for(let i = 0; i < amount.value; i += 1){
        let position1 = i + 1;
        const delays = Number(delay1.value) + step.value * i;
        createPromise(position1, delays).then(({ position , delay  })=>{
